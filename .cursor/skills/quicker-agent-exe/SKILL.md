...
找不到命令时，在 **`tools/qkagent/`** 执行 `pwsh -NoProfile -File ./publish/publish-agent.ps1`，新开终端后再试。
...
```powershell
# 编辑 tools/qkagent/actions/<sharedId>/page.html 后：
cd tools/qkagent
...
- PATH 中的 `qkagent`，或 `tools/qkagent/publish/agent/qkagent.exe`（整目录依赖）。
...
仓库源文件在 `tools/qkagent/.cursor/skills/`。同步到用户目录（可重复覆盖）：
