...
/// <summary>Loads embedded ActionDoc Markdown topics for qkagent CLI agents.</summary>
public sealed class ActionDocGuideService
{
  private const string ResourcePrefix = "QuickerAgent.AgentModel.Docs.ActionDoc.";

  private static readonly Lazy<IReadOnlyList<GuideTopic>> Topics = new(LoadTopics);

  public GetActionDocGuideResult GetDoc(string topic)
  {
    var key = NormalizeTopic(topic);
    if (string.IsNullOrEmpty(key))
    {
      return new GetActionDocGuideResult
      {
        Success = false,
  ...
  public SearchActionDocGuidesResult Search(string? keyword, int? maxResults)
  {
    ...
    var items = ordered
      .Select(x => new ActionDocGuideSearchItem
      {
        Topic = x.Topic.Topic,
        Title = x.Topic.Title,
    ...
    return new SearchActionDocGuidesResult
    {
      Success = true,
  ...
  {
    var assembly = typeof(ActionDocGuideService).Assembly;
    var names = assembly
  ...
  private static int ComputeSortScore(GuideTopic topic, string[] patterns)
  {
    ...
    foreach (var p in patterns)
    {
      ...
      if (topic.Markdown.Contains(p, StringComparison.OrdinalIgnoreCase))
      {
        score += 1;
      }
    }
  ...
  private sealed class GuideTopic(string topic, string title, string markdown)
  {
